customerUpdate.onshow=function(){
    ...
    if (req.status == 200) { //transit worked.
            allCustNames = JSON.parse(req.responseText)
            // names now in results array - load names into the dropdown
            selToChange.clear()
            for (i = 0; i <= allCustNames.length - 1; i++)
                selToChange.addItem(allCustNames[i])
    } else {
        // transit error
...
btnUpdate.onclick=function(){
    ...
    let found = false
    for (i = 0; i <= allCustNames.length - 1; i++) {
        // console.log(`FOUND IS false and name is ${allCustNames[i]}`)
        if (oldName == allCustNames[i]) {
            found = true
            break
        }
     }   
